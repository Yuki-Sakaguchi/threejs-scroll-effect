...
// スクロール追従
let targetScrollX = 0; // 本来のスクロール位置
let currentScrollX = 0; // 線形補完を適応した現在のスクロール位置
...
// スクロール位置を更新
const updateScroll = () => {
  targetScrollX = document.documentElement.scrollLeft; // スクロール位置を取得
  currentScrollX = lerp(currentScrollX, targetScrollX, 0.1); // リープ関数でスクロール位置をなめらかに追従
  scrollOffset = targetScrollX - currentScrollX; // 差分を保存
};
...
const resize = () => {
  ...
  document.body.style.width = `${scrollArea.getBoundingClientRect().width}px`;
};
...
const loop = () => {
  ...
  scrollArea.style.transform = `translate3d(${-currentScrollX}px,0,0)`;
